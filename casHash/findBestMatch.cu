...
struct MatchTraits : DescTraits<descElemType_, descDims_, distStyle_>{
	...
	struct Task
	{
		...
		const BucketDesc& query;
		const BucketDesc& train;
    ...
    inline static constexpr HW warpShape = {8, 4};
...
{
    using SMemInTileA = imma::SMemInTile<Traits::descElemType, Traits::descDims, Traits::distStyle, Traits::ctaTile.h>;
    using SMemInTileB = imma::SMemInTile<Traits::descElemType, Traits::descDims, Traits::distStyle, Traits::ctaTile.w>;
...
#pragma unroll
    ...
    __device__ __forceinline__
    void finishAcc(const uint32_t* sqrNormDescA, const Ptr<const uint32_t>& ctaDescIndicesA, const uint32_t* sqrNormDescB, const Ptr<const uint32_t>& ctaDescIndicesB)
    {
        const HW warpLocInCta = Traits::getWarpLocInCta();
        const auto warpDescIndicesA = ctaDescIndicesA + warpTile.h * warpLocInCta.h;
        const auto warpDescIndicesB = ctaDescIndicesB + warpTile.w * warpLocInCta.w;
        ...
        #pragma unroll
        for (int h = 0; h < warpTileUnroll.h; h++) {
            const auto atomDescIndicesA = warpDescIndicesA + warpTileAtom.h * h;
            #pragma unroll
            for (int i = 0; i < accAtomShape.h; i++) {
                const auto ptr = atomDescIndicesA + warpShape.h * i + Traits::getThrdLocInWarp().h;
                sqrNormA[h][i] = ptr.isInBound() ? sqrNormDescA[*ptr] : badSqrNormA;
            }
        }
        ...
        #pragma unroll
        for(int w = 0; w < warpTileUnroll.w; w++) {
            const auto atomDescIndicesB = warpDescIndicesB + warpTileAtom.w * w;
            for (int j = 0; j < accAtomShape.w; j++) {
                const auto ptr = atomDescIndicesB + accAtomShape.w * Traits::getThrdLocInWarp().w + j;
                sqrNormB[w][j] = ptr.isInBound() ? sqrNormDescB[*ptr] : badSqrNormB;
            }
        }
            ...
            for(int w = 0; w < warpTileUnroll.w; w++) {
                ...
                for (int i = 0; i < accAtomShape.h; i++) {
                    ...
                    for (int j = 0; j < accAtomShape.w; j++) {
                        ...
                        unused(coordInCta);
                        assert(sqrNormA[h][i] == ((ctaDescIndicesA + coordInCta.h).isInBound() ? sqrNormDescA[ctaDescIndicesA[coordInCta.h]] : badSqrNormA));
                        assert(sqrNormB[w][j] == ((ctaDescIndicesB + coordInCta.w).isInBound() ? sqrNormDescB[ctaDescIndicesB[coordInCta.w]] : badSqrNormB));
                    }
                }
            }
        }
...
#pragma unroll
    ...
    #pragma unroll
                for (int j = 0; j < accAtomShape.w; j++) {
                    for (unsigned x = warpShape.w; x < Traits::warp_size; x <<= 1) {
...
struct CtaTask : Traits
{
    ...
    {
		assert(!isBidirectional || task.results.train != nullptr);
    }
    const Task& task;
    ...
    void prologue() {
        ...
        const auto ctaDescIndicesA = cudapp::Ptr<const uint32_t>(task.query.bucket, task.query.bucketSize, idxDescBase.h);
        if (idxDescEndIfFull.h <= task.query.bucketSize) {
            smemA.ctaFill<ctaSize, true>(task.query.desc, ctaDescIndicesA);
        }
        else {
            smemA.ctaFill<ctaSize, false>(task.query.desc, ctaDescIndicesA);
        }
        const cudapp::Ptr<const uint32_t> ctaDescIndicesB {task.train.bucket, task.train.bucketSize, idxDescBase.w};
        if (idxDescEndIfFull.w <= task.train.bucketSize) {
            smemB.ctaFill<ctaSize, true>(task.train.desc, ctaDescIndicesB);
        }
        else {
            smemB.ctaFill<ctaSize, false>(task.train.desc, ctaDescIndicesB);
        }
        __syncthreads();
...
#pragma unroll
    ...
    void epilogue(){
        constexpr HW ctaTile = Traits::ctaTile;
        const HW idxDescBase = ctaTile * Traits::getCtaLocInGrid();
        const Ptr<const uint32_t> ctaDescIndicesA {task.query.bucket, task.query.bucketSize, idxDescBase.h};
        const Ptr<const uint32_t> ctaDescIndicesB {task.train.bucket, task.train.bucketSize, idxDescBase.w};
        warpTask.finishAcc(task.query.sqrNormDesc, ctaDescIndicesA, task.train.sqrNormDesc, ctaDescIndicesB);
...
#pragma unroll
        for (int n = 0; n < divUp(ctaTile.h, ctaSize); n++) {
            ...
            if (idxDescBaseA + idxDescCta < task.query.bucketSize) {
				const auto idxDescA = task.query.bucket[idxDescBaseA + idxDescCta];
				const auto idxDescB = task.train.bucket[ctaBestMatchA.index() + idxDescBaseB];
                reinterpret_cast<BestMatchGlobal&>(task.results.query[idxDescA]).updateRam(ctaBestMatchA.distance(), idxDescB);
            }
        }
        if (isBidirectional) {
#pragma unroll
            for (int n = 0; n < divUp(ctaTile.w, ctaSize); n++) {
                ...
                if (idxDescBaseB + idxDescCta < task.train.bucketSize) {
					const auto idxDescB = task.train.bucket[idxDescBaseB + idxDescCta];
					const auto idxDescA = task.query.bucket[ctaBestMatchB.index() + idxDescBaseA];
                    reinterpret_cast<BestMatchGlobal&>(task.results.train[idxDescB]).updateRam(ctaBestMatchB.distance(), idxDescA);
                }
            }
        }
    }
    ...
    void prefetchDescSqrNorm(){
        ...
        constexpr HW ctaTile = Traits::ctaTile;
        const HW idxDescBase = ctaTile * Traits::getCtaLocInGrid();
        prefetch(task.query.bucket + idxDescBase.h, int(task.query.bucketSize) - idxDescBase.h, ctaTile.h);
        prefetch(task.train.bucket + idxDescBase.w, int(task.train.bucketSize) - idxDescBase.w, ctaTile.h);
    }
};

constexpr uint32_t maxNbTasks = 16;
...
#endif
kernelFindBestMatch(const std::array<ImageDesc, maxNbTasks> queries, const ImageDesc train, const std::array<MatchResult, maxNbTasks> results, const uint32_t nbTasks)
{
	const uint32_t idxQuery = blockIdx.z / (nbBuckets * nbScheme);
	assert(idxQuery < nbTasks);
	constexpr auto ctaTile = Traits::ctaTile;

	const ImageDesc& query = queries[idxQuery];
	const MatchResult& result = results[idxQuery];

	const uint32_t idxScheme = blockIdx.z % (nbBuckets * nbScheme) / nbBuckets;
	const uint32_t idxBucket = blockIdx.z % nbBuckets;
	...
	const uint32_t queryBucketSize = query.bucketSize(idxScheme, idxBucket);
	const uint32_t trainBucketSize = train.bucketSize(idxScheme, idxBucket);
    const HW idxDescBase = ctaTile * Traits::getCtaLocInGrid();
    if (idxDescBase.h >= queryBucketSize || idxDescBase.w >= trainBucketSize) {
        return;
    }

	const typename Traits::Task task{
		{query.desc, query.sqrNormDesc, &query.buckets[query.bucketBeg(idxScheme, idxBucket)], queryBucketSize},
		{train.desc, train.sqrNormDesc, &train.buckets[train.bucketBeg(idxScheme, idxBucket)], trainBucketSize},
		result
	};
...
template <typename Traits, uint32_t ctaSizeForInit>
__global__ void kernelInitCasHashMatch(const std::array<MatchResult, maxNbTasks> results, std::array<uint32_t, maxNbTasks> querySize, const uint32_t trainSize, uint32_t nbTasks) {
    const auto idxTask = blockIdx.z;
    assert(idxTask < results.size());
    const auto& task = results[idxTask];
    const unsigned x = ctaSizeForInit * blockIdx.x + threadIdx.x;
    if (x < querySize[idxTask]) {
        reinterpret_cast<typename Traits::BestMatchGlobal&>(task.query[x]).init();
    }
    if (Traits::bidirectional && x < trainSize) {
		assert(task.train != nullptr);
        reinterpret_cast<typename Traits::BestMatchGlobal&>(task.train[x]).init();
    }
}

template <typename Traits>
void findBestMatch(const std::array<ImageDesc, maxNbTasks>& queries, const std::array<uint32_t, maxNbTasks>& querySize, const ImageDesc& train, uint32_t trainSize,
	const std::array<MatchResult, maxNbTasks>& results, uint32_t nbTasks,
	uint32_t maxQueryBucketSize, uint32_t maxTrainBucketSize, cudaStream_t stream)
{
	ASSERT(inRange(nbTasks, 0u, maxNbTasks + 1));
	{
		const uint32_t ctaSizeForInit = 128;
		const uint32_t maxQuerySize = *std::max_element(querySize.begin(), querySize.end());
		const dim3 dimGridInit {divUp(max(trainSize, maxQuerySize), ctaSizeForInit), 1u, nbTasks};
		kernelInitCasHashMatch<Traits, ctaSizeForInit><<<dimGridInit, ctaSizeForInit, 0, stream>>>(results, querySize, trainSize, nbTasks);
		cudaCheck(cudaGetLastError());
	}
	{
		dim3 dimGrid{divUp(maxTrainBucketSize, Traits::ctaTile.w), divUp(maxQueryBucketSize, Traits::ctaTile.h), nbBuckets * nbScheme * nbTasks};
		kernelFindBestMatch<Traits><<<dimGrid, Traits::ctaSize, 0, stream>>>(queries, train, results, nbTasks);
		cudaCheck(cudaGetLastError());
	}
}
...
{
	const bool bidirectional = results[0].train != nullptr;
	for (uint32_t idxBeg = 0u; idxBeg < nbTasks; idxBeg += maxNbTasks) {
		const uint32_t batchSize = std::min(nbTasks - idxBeg, maxNbTasks);
		auto makeArray = [&](auto* p) {
			using Elem = std::decay_t<decltype(*p)>;
			std::array<Elem, maxNbTasks> ret;
			std::copy_n(p + idxBeg, batchSize, ret.begin());
			return ret;
		};
		const auto batchMaxQueryBucketSize = *std::max_element(maxQueryBucketSize + idxBeg, maxQueryBucketSize + idxBeg + batchSize);
		if (bidirectional) {
			using Traits = TraitsIMMASift8b;
			findBestMatch<Traits>(makeArray(queries), makeArray(querySize), train, trainSize, makeArray(results), batchSize, batchMaxQueryBucketSize, maxTrainBucketSize, stream);
		}
		else {
			using Traits = MatchTraits<DescElemType::kU8, 128, DistanceStyle::kL2, true, 16, 8, 32>;
			findBestMatch<Traits>(makeArray(queries), makeArray(querySize), train, trainSize, makeArray(results), batchSize, batchMaxQueryBucketSize, maxTrainBucketSize, stream);
		}
	}
}
